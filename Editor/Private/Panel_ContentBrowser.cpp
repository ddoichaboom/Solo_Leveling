...
void CPanel_ContentBrowser::Render_Files()
{
	if (m_bNeedRefresh)
		Refresh();
...
void CPanel_ContentBrowser::Render_Prototypes()
{
	auto RenderGroup = [this](const _char* szLabel, _uint iLevel)
		{
			vector<PROTOTYPE_INFO> Infos;
			if (FAILED(m_pGameInstance->Enum_Prototypes(iLevel, Infos)))
				return;

			// GAMEOBJECT 만 필터링
			_uint iCount = 0;
			for (const auto& Info : Infos)
				if (Info.eType == PROTOTYPE::GAMEOBJECT)
					++iCount;

			_char szHeader[MAX_PATH] = {};
			sprintf_s(szHeader, "%s (%u)", szLabel, iCount);

			if (!ImGui::CollapsingHeader(szHeader, ImGuiTreeNodeFlags_DefaultOpen))
				return;

			ImGui::PushID(static_cast<int>(iLevel));
			for (const auto& Info : Infos)
			{
				if (Info.eType != PROTOTYPE::GAMEOBJECT)
					continue;

				const string strTag = WTOA(Info.strTag);

				ImGui::Selectable(strTag.c_str());

				// DnD source — Step 13 에서 Viewport 가 받음
				if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID))
				{
					DND_PROTOTYPE Payload{};
					Payload.iLevel = iLevel;
					wcsncpy_s(Payload.szTag, Info.strTag.c_str(), _TRUNCATE);

					ImGui::SetDragDropPayload(DND_PROTOTYPE_TYPE, &Payload, sizeof(Payload));
					ImGui::Text("%s", strTag.c_str());
	...
	// STATIC (level 0) — 항상 노출
	RenderGroup("[STATIC]", 0);

	// 현재 레벨 — STATIC 과 다를 때만
	_int iCurLevel = m_pGameInstance->Get_CurrentLevelIndex();
	if (iCurLevel > 0)
	{
		_char szLabel[64] = {};
		sprintf_s(szLabel, "[Level %d]", iCurLevel);
		RenderGroup(szLabel, static_cast<_uint>(iCurLevel));
	}
}
