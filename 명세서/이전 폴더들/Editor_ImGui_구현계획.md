...
→ **(b) Editor 내부** 채택. CPanel_Viewport가 RT 리소스를 직접 소유.

**✅ 구현 완료 — CPanel_Viewport 멤버:**
- `ID3D11Texture2D* m_pRTTexture` + `ID3D11RenderTargetView* m_pRTV` + `ID3D11ShaderResourceView* m_pSRV`
- `ID3D11Texture2D* m_pDSTexture` + `ID3D11DepthStencilView* m_pDSV`
- `D3D11_VIEWPORT m_Viewport`, `m_iRTWidth`, `m_iRTHeight`
- `Create_RenderTarget()` / `Release_RenderTarget()` / `Begin_RT()` / `End_RT()`
...
```
[1] Render_Scene():
    → m_pViewport->Begin_RT()  (별도 RTV/DSV 바인딩 + Clear)
    → m_pGameInstance->Draw()  (3D 오브젝트 → 별도 RenderTarget)
    → m_pViewport->End_RT()    (RT 바인딩 해제, null로)

[2] ImGui 렌더 패스:
    → Begin_Draw()  (BackBuffer Clear + OMSetRenderTargets 재바인딩)
    → ImGui NewFrame → DockSpace → 패널들
    → Viewport 패널: ImGui::Image(reinterpret_cast<ImTextureID>(SRV), 패널크기)
    → Render_ImGui() → End_Draw (Present)
```
**주의**: `CGraphic_Device::Clear_BackBuffer_View()`에 `OMSetRenderTargets` 재바인딩 추가됨 (별도RT→BackBuffer 복원 보장)
...
- [x] 별도 RenderTarget 생성/해제 정상 동작
- [x] 3D 씬이 Viewport 패널 안에만 표시됨 (Render_Scene → Begin_RT/End_RT 파이프라인)
- [x] Viewport 패널 리사이즈 시 3D 씬 비율 유지
- [x] 다른 패널이 Viewport를 가리지 않음
...
- **C++17 필수**: Editor 프로젝트 속성 → C/C++ → 언어 → `/std:c++17`
- `std::filesystem::directory_iterator`로 디렉토리 순회 + 벡터 캐싱 (m_Directories, m_Files)
- `m_bNeedRefresh` 플래그로 경로 변경 시에만 재순회 (매 프레임 순회 방지)
- `ImGui::Selectable()`로 파일/폴더 목록, 더블클릭 폴더 진입
- Breadcrumb: 경로 조각별 `ImGui::Button` + `<-` 뒤로가기, Root(Resources/) 이탈 방지
- 파일 타입 아이콘: `Get_FileIcon()` — [D]폴더 [T]텍스처 [S]셰이더 [M]모델 [B]바이너리 [?]기타
- 이름순 `std::sort` 정렬 (폴더/파일 각각)
- 썸네일 표시는 후순위 (우선 텍스트 목록)

### 3-3. 완료 기준

- [x] Resources/ 하위 폴더/파일 목록 표시
- [x] 폴더 더블클릭으로 진입/상위 이동
- [x] 파일 선택 상태 관리
...
```
Phase 1: Viewport 분리 (별도 RenderTarget)     ✅ 완료
Phase 2: 기본 패널 구조 (5개 패널 껍데기)         ✅ 완료
Phase 3: Content Browser                       ✅ 완료
Phase 4: Hierarchy + Inspector + RTTR          ← 다음 작업
