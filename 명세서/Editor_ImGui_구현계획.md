...
**CPanel_Manager (패널 매니저):**

패널을 `map<wstring, CPanel*>`로 관리. 이름 기반 접근으로 패널 간 통신 지원.
CGameObject의 Component 패턴(Add_Component → map 등록 + 멤버 포인터 캐싱)과 동일한 구조.
...
m_pPanel_Manager->Add_Panel(TEXT("Panel_Hierarchy"), CPanel_Hierarchy::Create(m_pDevice, m_pContext));
m_pPanel_Manager->Add_Panel(TEXT("Panel_Inspector"), CPanel_Inspector::Create(m_pDevice, m_pContext));
m_pPanel_Manager->Add_Panel(TEXT("Panel_ContentBrowser"), CPanel_ContentBrowser::Create(m_pDevice, m_pContext));
m_pPanel_Manager->Add_Panel(TEXT("Panel_Log"), CPanel_Log::Create(m_pDevice, m_pContext));
...
// Menu Bar에서 토글 — Get_Panel로 접근
if (ImGui::BeginMenu("Window"))
{
    for (auto& [tag, pPanel] : m_pPanel_Manager->Get_All())
        ImGui::MenuItem(pPanel->Get_Name(), nullptr, &pPanel->Is_Open_Ref());
    ImGui::EndMenu();
}
```
...
이후에는 `imgui.ini`에 저장되어 사용자 커스텀 레이아웃 유지.

### 2-4. Menu Bar

DockSpace 상단에 메뉴바 추가:
```
File: New Scene / Open Scene / Save Scene / Exit
Edit: Undo / Redo (향후)
Window: Hierarchy / Inspector / Content Browser / Log / Viewport (토글)
Tools: Model Converter (Phase 별도)
```

### 2-5. 완료 기준

- [ ] 5개 패널이 DockSpace 내 기본 레이아웃으로 배치됨 (Log은 Content Browser와 탭)
- [ ] 각 패널 자유롭게 도킹/탭/리사이즈 가능
- [ ] Menu Bar에서 패널 표시/숨기기 토글
- [ ] imgui.ini로 레이아웃 저장/복원
