...
#### Layer 1 진행 현황 (2026-04-11 갱신 — **완료, 5-C 보류**)
...
| **5** | **Inspector 확장** | **완료** (5-A/5-B/5-D 완료 / **5-C 보류 → Player·AnimController 단계에서 재설계**) | 아래 상세 |
...
| 5-C | **애니메이션 전환 보간 (블렌딩)** | **Engine 기능 + Inspector UI** | **보류** — Player/Weapon·AnimController 단계에서 재설계 |
...
**5-C 애니메이션 전환 블렌딩 — 보류 결정 (2026-04-11):**

- **현재 상태**: `Set_AnimationIndex`는 **즉시 전환 (하드 컷)**. 블렌딩 관련 Engine/Editor 코드는 모두 제거됨
- **1차 시도 결과 (2026-04-10)**: CrossFade(SQT 동시 재생 Lerp) 방식으로 구현했으나 아래 이슈들 발견:
  1. 비채널 본(채널이 없는 본)까지 `XMMatrixDecompose → XMMatrixAffineTransformation` 라운드트립으로 덮어써서 정밀도 오차 누적 → 본 어긋남
  2. 한쪽에만 채널이 있는 본은 블렌드 종료 후에도 잘못된 재구성 행렬이 잔존
  3. 루트 본 Translation 키프레임이 있는 애니메이션은 A/B 동시 전진 중 월드 위치가 이중 이동
  4. 구조 복잡도(BlendPrev/Curr SQT, BindPose 캐시, Update_SQT, Dirty Mask 등)가 계속 증가하며 유지보수 부담 ↑
- **보류 결정 배경**:
  - 블렌딩은 **"언제 어떻게 전환할 것인가"** 의 고수준 제어(상태머신, 파라미터, 전환 조건)와 떼어놓고 설계하면 재작업 가능성이 큼
  - Solo Leveling 프로젝트는 이후 **플레이어 클래스 + 무기 클래스 + 애니메이션 컨트롤러**를 구현할 예정이며, 그 시점에 블렌딩 방식(CrossFade / Additive / Masked / Blend Tree)을 상태머신 구조와 **함께** 설계하는 것이 자연스러움
  - 에디터 프리뷰 용도에는 즉시 전환(하드 컷)으로도 충분
- **재시도 시점**:
  1. **Layer 2 완료 후** — 씬 배치/저장 인프라가 준비된 상태에서
  2. **플레이어/무기 클래스 구현 시** — 애니메이션 컨트롤러 설계와 동시 진행
- **재시도 시 참고사항**:
  - 1차 시도 제거 범위: BONE_SQT 구조체, CChannel::Get_SQT, CAnimation::Update_SQT, CModel 블렌딩 멤버(m_iPrevAnimIndex / m_fBlendElapsed / m_fBlendDuration / m_isBlending / m_BindPoseSQT / m_BlendPrevSQT / m_BlendCurrSQT) 및 관련 getter, Cache_BindPoseSQT, Play_Animation 블렌딩 분기, Set_AnimationIndex 블렌드 트리거 분기, Inspector 블렌드 UI
  - **비채널 본은 절대 decompose/recompose 시키지 말 것** — 원본 `m_TransformationMatrix`(aiNode mTransformation 그대로)를 유지해야 정밀도 손실 없음
  - **"Freeze A → Transition to B[0] → Play B" 방식 검토 가치 있음** — A 전진 중단 + B는 keyframe[0]에 고정, 그 사이만 Lerp. 동시 재생 방식보다 구조가 단순하고 루트 모션 이중 이동 없음. 단 상태머신 설계와 맞물려 재검토 필요
  - 상태머신 도입 시 CrossFade는 "전환 엣지의 한 속성"으로 자연스럽게 녹아듦

**🔴 Layer 1 보류 과제 (Player/Weapon·AnimController 단계에서 재진입):**

| 과제 | 설명 | 의존 관계 |
|---|---|---|
| **5-C 애니메이션 블렌딩** | 위 참조. 상태머신과 함께 재설계 | AnimationController 선행 필요 |
| **루트 모션 위치 동기화** | 본 애니메이션은 로컬 공간 동작, CTransform 월드 위치는 갱신 안 됨 → 루트 본 Translation 키프레임이 있으면 메쉬만 이동 + 전환 시 스냅백. 해결: 매 프레임 루트 본 delta → CTransform 적용, 루트 본은 원점 고정 | 블렌딩과 독립. 블렌딩보다 먼저 해결 가능 |
...
**진입 시점**: 2026-04-11 — Layer 1 완료 (5-C 블렌딩 보류) 후 다음 단계로 확정
**브랜치**: `feature/ImGuizmo` (Layer 2-a ImGuizmo 통합 범위)

**후속 연계 (미리 메모)**:
- Layer 2 이후 플레이어/무기 클래스 + 애니메이션 컨트롤러 구현 예정
- 그 시점에 Layer 1 보류 과제(5-C 블렌딩, 루트 모션)를 재진입 → 상태머신 설계와 함께 재설계
- Layer 2의 씬 배치 인프라가 완성되어 있어야 플레이어 테스트 환경이 확보됨 (Layer 2 선행 이유)

**목적:** 3D 오브젝트 배치/편집 + 씬 저장/로드

**Layer 2-a: 기본 배치**
- 오브젝트 선택 (피킹으로 클릭) — ✅ Layer 0에서 완료
- **ImGuizmo Transform 조작 (이동/회전/스케일)** — ✅ Step 1~6 완료 (2026-04-11)
- 스냅/그리드 여부 결정 — 다음 세션
- QA 시나리오 — 다음 세션
...
#### Layer 2-a ImGuizmo 통합 진행 현황 (2026-04-11)

| Step | 내용 | 상태 |
|------|------|------|
| 1 | 라이브러리 세팅 — `ImGuizmo::SetImGuiContext` + `BeginFrame` 연동 | ✅ 완료 |
| 2 | 행렬 연동 인터페이스 — `CTransform::Set_WorldMatrix` 인라인 setter (Engine 유일 변경점) | ✅ 완료 |
| 3 | Viewport 오버레이 연동 — `SetDrawlist` + `SetRect(vImagePos, iRTWidth, iRTHeight)` | ✅ 완료 |
| 4 | 선택 상태 연동 — `Manipulate(View,Proj,op,mode,world)` + `bGizmoBlocking` 피킹 가드 | ✅ 완료 |
| 5 | 조작 모드 & 단축키 + `CPanel_Shortcuts` 신규 패널 | ✅ 완료 |
| 6 | Inspector 양방향 동기화 — `BeginDisabled`/`EndDisabled` 가드 (폴링 기반) | ✅ 완료 |
| 7 | 스냅/그리드 여부 결정 | 다음 세션 |
| 8 | QA 시나리오 | 다음 세션 |

**주요 설계 결정사항:**

- **Engine API 최소 변경**: Layer 2-a 전체에서 Engine 은 `Set_WorldMatrix(const _float4x4&)` 인라인 setter **한 줄만** 추가. ImGuizmo 의존성은 Editor 에만 격리 (Assimp/RTTR 격리 원칙과 일관)
- **DirectX 행우선 ↔ ImGuizmo 호환**: `_float4x4` 를 `reinterpret_cast<float*>` 로 넘기면 별도 transpose 없이 그대로 동작. DirectX row-major + pre-multiplication 규약이 ImGuizmo 기본 규약과 일치
- **폴링 기반 양방향 동기화**: Inspector RTTR getter 와 ImGuizmo 가 독립적으로 매 프레임 `m_WorldMatrix` 를 읽고 씀. Single Source of Truth → 이벤트 시스템/dirty 플래그 불필요. 실제로 필요했던 변경은 `Render_Transform` 에 `BeginDisabled` 가드 추가 한 곳
- **기즈모 ↔ 피킹 충돌 해결**: `_bool bGizmoBlocking` 를 Manipulate 블록 **바깥**에서 선언하고 `pSelected && pTransform` 브랜치 내부에서만 `IsOver() || IsUsing()` 대입. 선택 해제 직후에 남는 stale hit-test 방지
- **RMB-modal 키 충돌 공존**: CCamera_Free 는 RMB 누를 때만 WASD/QE 카메라 조작. ImGuizmo 단축키는 `!ImGui::IsMouseDown(ImGuiMouseButton_Right)` 가드로 RMB 가 아닐 때만 처리 → 키 매핑 재설계 없이 배타적 공존
- **단축키를 Render() 에 배치**: `ImGui::IsWindowFocused()` 는 해당 창 `Begin()`/`End()` 스코프 내에서만 유효. Update() 에서는 호출 불가. ImGui 이디엄상 패널 Render() 는 프레임 핸들러 역할 (입력 + 드로잉 혼재가 정상 패턴)
- **CPanel_Shortcuts 별도 패널**: Viewport 내부 드로리스트 오버레이 대신 분리된 패널로 구현. DockBuilder 에 통합 + 숨김/표시 토글 지원. `bool bCameraMode = ImGui::IsMouseDown(ImGuiMouseButton_Right);` 로 매 프레임 재평가하여 RMB 상태에 따라 활성 모드 강조 표시

**단축키 매핑 (Viewport 포커스 + RMB OFF 시):**
...
- **Euler 회전 라운드트립 불가역성**: WorldMatrix → Euler 분해는 다중값 해석이 존재. 기즈모 회전 후 Inspector 의 Degree 표시는 수학적으로 동등한 다른 조합이 나올 수 있음 — Inspector 를 "숫자 미세 조정" 용도로 보는 현 설계에서는 허용
- **Scale 부호 손실**: Scale getter 가 행 길이(`XMVector3Length`) 로 계산 → 항상 양수. 거울 반전(음수 스케일) 표현 불가 — 사용 시나리오에 음수 스케일 없음
- **ImGuizmo SCALE 은 항상 LOCAL**: 사양상 제약

**Layer 2-a 변경 파일 목록:**

| 분류 | 파일 | 변경 |
|---|---|---|
| Engine | `Engine/Public/Transform.h` | `Set_WorldMatrix(const _float4x4&)` 인라인 setter 추가 |
| Editor | `Editor/Public/Editor_Defines.h` | `#include "ImGuizmo.h"` |
| Editor | `Editor/Private/EditorApp.cpp` | `SetImGuiContext` (Ready_ImGui), `BeginFrame` (Begin_ImGuiFrame), Panel_Shortcuts 등록/도킹/메뉴 |
| Editor | `Editor/Public/Panel_Viewport.h` | `m_eGizmoOperation`, `m_eGizmoMode` 멤버 |
| Editor | `Editor/Private/Panel_Viewport.cpp` | SetDrawlist/SetRect, 단축키, Manipulate, bGizmoBlocking 가드 |
| Editor | `Editor/Public/Panel_Shortcuts.h` | **신규** CPanel 파생 final 클래스 |
| Editor | `Editor/Private/Panel_Shortcuts.cpp` | **신규** RMB 기반 컨텍스트 민감 Render |
| Editor | `Editor/Public/Panel_Manager.h` | `#include "Panel_Shortcuts.h"` |
| Editor | `Editor/Private/Panel_Inspector.cpp` | `Render_Transform` 을 `IsUsing()` 기반 BeginDisabled/EndDisabled 가드로 감싸기 |

#### Layer 2-a 후속 점검: Player / PartObject / Weapon 테스트 (2026-04-12)

**배경:** Scene Loader와 JSON 구조를 확정하기 전에, 임시 `CModelObject`만으로는 실제 게임 오브젝트 구조를 판단하기 어렵다. 수업 코드의 ContainerObject/PartObject 패턴을 그대로 복붙하기보다, 현재 프로젝트 기준으로 필요한 관계만 먼저 흡수하여 Player/Body/Weapon 테스트 구조를 만들고 검증한다.

**현재 구현/검증된 흐름:**

- `CContainerObject`
  - `m_PartObjects` map으로 파츠를 소유
  - `Add_PartObject()` 로 GAMEOBJECT prototype을 Clone하여 파츠로 등록
  - Editor 피킹을 위해 `Get_PartObjects()` 접근자 필요
- `CPartObject`
  - `PARTOBJECT_DESC::pParentMatrix` 로 부모 WorldMatrix 포인터를 받음
  - `Compute_CombinedWorldMatrix()` 로 자식 로컬/소켓 행렬과 부모 행렬을 합성
  - Body/Weapon은 Render 시 `m_CombinedWorldMatrix` 를 shader world matrix로 사용
- `CPlayer`
  - `CContainerObject` 파생
  - `Body` 파츠 생성 후 Body의 본 포인터를 가져와 `Weapon` 파츠 생성
  - 현재 소켓 본: `"Prop_Weapon_Dualwield_01_R"`
- `CBody_Player`
  - `CPartObject` 파생
  - `Prototype_Component_Shader_VtxAnimMesh` + `Prototype_Component_Model_SungJinWoo`
  - `Get_BoneMatrixPtr()` 로 Weapon이 붙을 소켓 본 행렬 제공
  - Inspector 애니메이션 미리보기와 충돌하지 않도록 state-driven `Set_AnimationIndex(0/1)` 는 임시 비활성화 상태
- `CWeapon`
  - `CPartObject` 파생
  - `Prototype_Component_Shader_VtxMesh` + `Prototype_Component_Model_Weapon01`
  - `pSocketBoneMatrix` 를 받아 Late_Update에서 소켓 행렬 축 정규화 후 CombinedWorldMatrix 계산
- `Panel_Viewport`
  - 일반 GameObject 피킹 후, `CContainerObject` 내부 `CPartObject` 의 `Com_Model` 도 순회
  - `CPartObject::Get_CombinedWorldMatrix()` 기준으로 CPU skinned/static model picking 수행
  - Body/Weapon을 직접 선택하면 Inspector에서 해당 파츠의 모델 정보 및 애니메이션 목록 확인 가능

**확인된 문제와 판단:**

- Inspector에서 애니메이션 클릭은 `CModel::Set_AnimationIndex()` 까지 도달함
- 다만 `CBody_Player::Update()` 에서 Player 상태 기반 `Set_AnimationIndex(0/1)` 를 매 프레임 호출하면 Inspector 선택이 즉시 덮어써짐
- 따라서 Editor preview 모드와 gameplay animation state machine을 분리해야 함
- 단기 확인은 state-driven 애니메이션 전환 임시 비활성화로 충분
- 장기 구조는 `CModel` 내부가 아니라 Player/Body 쪽 AnimationController 또는 AnimationResolver가 책임져야 함

#### Player 애니메이션 관리 방향 (2026-04-12)

**문제:** 현재 캐릭터 애니메이션은 약 47개이며, 단순 Animation List 나열과 직접 인덱스 참조로는 관리가 어렵다. 일부 애니메이션은 이동/스킬/공격용이고, 일부는 무기 장착 상태에서만 사용하며, `Fighter` 계열처럼 무기 없이 재생해야 하는 애니메이션도 존재한다.

**결정 방향:** 애니메이션 인덱스를 gameplay 코드에 직접 박지 않는다. 의미 있는 액션 enum과 무기 상태 enum을 기준으로 AnimationName을 매핑하고, 초기화 시 이름을 index로 캐싱한다.
...
- Input은 직접 animation index를 고르지 않고 `PLAYER_ACTION` 만 결정
- `PLAYER_ACTION + WEAPON_STATE` 조합으로 실제 애니메이션을 선택
- `bShowWeapon` 으로 Weapon 파츠 렌더 여부를 함께 결정
- `Fighter` 같은 문자열 포함 규칙은 초기 자동 분류/검증 힌트로만 사용하고, 최종 gameplay 로직은 명시적 매핑 테이블 또는 JSON 데이터에 의존
- Inspector Animation List는 장기적으로 검색, 카테고리, 무기 상태 필터를 제공해야 함

**Scene JSON으로 이어질 결정사항:**

- Scene JSON의 `desc` 는 단순 모델 경로만 담지 않고, Player 같은 컨테이너형 오브젝트의 파츠 구성과 소켓 이름을 표현할 수 있어야 함
- 단, 액션별 애니메이션 매핑은 씬 배치 정보라기보다 캐릭터/프리팹/애니메이션 컨트롤러 데이터에 가깝다
- Scene Loader 1차에서는 Player 인스턴스 배치와 transform, prototype tag, layer 중심으로 시작하고, AnimationTable은 별도 캐릭터 데이터로 분리하는 편이 안전
