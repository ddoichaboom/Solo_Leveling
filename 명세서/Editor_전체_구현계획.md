...
| 항목 | 내용 | 분류 |
|---|---|---|
| 5-A | 모델 기본 정보 (타입, 메시/머티리얼/본/애니메이션 개수) | Inspector UI |
| 5-B-1 | 애니메이션 리스트 (이름 목록, 클릭 전환) | Inspector UI |
| 5-B-2 | Loop 토글 (현재 애니메이션 bIsLoop 표시 + 런타임 변경) | Inspector UI + Engine getter/setter |
| 5-B-3 | 타임라인 정보 (CurrentTrackPosition / Duration, 진행률 바, 키프레임 위치) | Inspector UI |
| 5-C | **애니메이션 전환 보간 (CrossFade 블렌딩)** | **Engine 기능 + Inspector UI** |
| 5-D | **모델 피킹** | **Engine CModel::Pick() + CGameObject 피킹 경로 확장** |
...
- **방식 — CrossFade 블렌딩**:
  ```
  A 애니메이션 (fadeout)  ──┐
                            ├──  최종 본 행렬 = lerp(A결과, B결과, blendRatio)
  B 애니메이션 (fadein)   ──┘
  blendRatio: 0→1 (BlendDuration 동안 선형 증가)
  ```
- **Engine 변경 범위 (예상)**:
  - CModel: `m_iPrevAnimIndex`, `m_fBlendTime`, `m_fBlendDuration`, `m_isBlending` 추가
  - Play_Animation 수정: 블렌딩 중이면 이전/현재 두 애니메이션 모두 Update → 본별 보간
  - Set_AnimationIndex 수정: 즉시 전환 대신 블렌드 시작 트리거
  - 본별 최종 행렬 보간: SQT 분해 후 Slerp(회전)/Lerp(위치·스케일) 또는 행렬 직접 Lerp
- **Inspector 노출**: Blend Duration 슬라이더, 블렌딩 상태 표시
- **구현 시점**: Step 5에서 함께 할지, Layer 2로 미룰지 — **구체화 단계에서 최종 결정**

**5-D 모델 피킹 — 설계 결정사항 (논의 완료):**

- **기존 인프라 재사용 가능**: CMesh는 CVIBuffer를 상속하며, Initialize에서 **이미 PICK_DATA를 생성**하고 있음 (pVerticesPos + pIndices CPU 사본). CVIBuffer::Pick() (TriangleTests::Intersects) 도 상속
- **현재 문제**: CModelObject의 m_pVIBufferCom = nullptr → Panel_Viewport 피킹 경로에서 무시됨
- **해결 방향**:
  1. CModel에 `Pick(vRayOrigin, vRayDir, pDist)` 추가 — 전체 메시 순회, 각 CMesh::Pick() 호출, 최소 거리 반환
  2. CGameObject 피킹 경로 확장: m_pVIBufferCom 없으면 CModel 컴포넌트 확인 → CModel::Pick() 호출
- **정확도**: 삼각형 레벨 (바운딩 박스 대신 기존 인프라 그대로 사용)
- **성능**: 하이폴리에서 느릴 수 있으나 YAGNI — 필요 시 바운딩 박스 사전 체크 추가
- **구현 시점**: Step 5에서 함께 진행 (기존 인프라 재사용이라 작업량 적음)
