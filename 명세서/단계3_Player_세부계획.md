...
> ⚠️ **Player 1차 마감 2026-05-05** — 후속 문서: `명세서/통합_구현계획_v3.md`
> Player 이동/공격/가드 1차 작업이 완료되었고 (Step A / B / C-1~C-7), 잔여 항목 (Step C-8 LINK/CHAIN, Step D 무기 스왑, Step E 카메라+Style C, 단계 2-3 블렌딩) 은 v3 의 트랙 1·5 로 재배치되었다. 본 문서는 v3 §1.2 에 잔여만 흡수된 후 **참조용으로 보존** — 신규 결정은 v3 에 기록.
>
> ---
>
> 작성일: 2026-04-22
...
- Dash/BackDash 가 **이동 입력 유무 기반**으로 분기 (WASD 누른 상태 + SPACE → DASH, SPACE 단독 → BACK_DASH). 2026-04-26 정책 변경 — §4.4.1 참조
...
#### 4.4.1 SPACE 문맥 해석 (2026-04-26 갱신 — 이동 입력 유무 기반 분기)
...
if (CurrentAction ∈ {BASIC_ATTACK_01~03, LINK_ATTACK, CHAIN_SMASH}):
    ...
    if (Intent.bHasMoveIntent == true):
        → Intent::DashForward                   // WASD 누른 상태 + SPACE
        // StateMachine 측에서 Player::Face_DirectionImmediately(vMoveDirWorld)
        // 호출하여 카메라-상대 이동 방향으로 즉시 회전 후 DASH 진입
    else:
        → Intent::DashBackward                  // SPACE 단독 (이동 입력 없음)
else:
    → 무시 (GUARD/DASH/HIT 중 SPACE 는 무효)
```

**정책 변경 배경 (2026-04-26)**:
- 이전: `MoveVector.forward < 0` (= S 키 포함) → BACK_DASH. 키-조합 기반
- 변경: `bHasMoveIntent` 만 기준. **`S` 단독 + SPACE 도 DASH** (회전 후 후방 이동)
- 이유:
  1. "정면 바라보기 + Dash" 시 카메라/플레이어 yaw 차이 π 근처로 누적되던 stale frame 증상이 가장 빈번한 시나리오(WASD+Space)에서 사라짐 — 4/25 §3 잔존 이슈 부분 해소
  2. 의도 명확 — "어디론가 가고 싶다(WASD) → DASH", "제자리에서 회피만(SPACE 단독) → BACK_DASH"
  3. S+SPACE 가 BACK_DASH 라는 키-조합 규칙의 모호성 제거 (S+D+SPACE 같은 조합이 자연스럽게 처리됨)
...
IDLE ──(WASD 누름)──────→ WALK
WALK ──(WASD 뗌)────────→ IDLE
WALK/IDLE ──(SPACE, 차지 소비)─→ DASH (WASD 홀드, 이동 방향 회전 후) / BACK_DASH (SPACE 단독)
...
Step B. 상태머신 골격 도입. IDLE/WALK/RUN/DASH/BACK_DASH 만 우선.
        SPACE 입력을 Intent 로 해석 (이동 입력 유무로 DASH/BACK_DASH 분기, §4.4.1).
...
**Dash/콤보/LINK/CHAIN**
- [ ] `WASD + SPACE` → DASH (이동 방향으로 즉시 회전 후 진입), Root Motion 누적
- [ ] `SPACE` 단독 (이동 입력 없음) → BACK_DASH, Root Motion 누적
- [ ] `S + SPACE` → DASH (S 만 눌린 상태도 이동 입력 → 회전 후 후방 DASH). 더 이상 BACK_DASH 아님
...
### 12.0 Player 1차 마감 안내 (2026-05-05)

Step A / B / C-1 ~ C-7 까지 완료. **Player 기본 이동 + 기본 공격 + 가드까지를 1차로 마감**하고, 다음 트랙은 `명세서/통합_구현계획_v2.md` §4 기준으로 재우선순위화한다.

**미완 / 보류 항목** (다음 트랙 결정 시 재배치):
- ⏸ **Step C-8** (LINK_ATTACK / CHAIN_SMASH) — 스킬 시스템 구현 시점에 함께 진행하기로 보류
- ⏸ **Step C 1차 커밋** (콤보 + 가드 + AnimNotify 인프라, LINK 제외) — 다음 작업 진입 직전에 커밋 예정 (작업 트리 정리용)
- ⏸ **Step D 전체** (D-1 ~ D-6, 무기 스왑 + 쿨다운) — 우선순위 재검토. Step C-8 과 함께 스킬 단계로 묶을지 별도 진행할지 다음 트랙 결정 시 함께 판단
- ⏸ **Step E** (추적 카메라 + Style C 이동 해석) — 다음 트랙 후보 중 하나로 유지

**알려진 후속 (Player 단계 마감 후 처리)**:
- Guard_Dagger 클립 무기 회전 시각 이슈 (memory: `project_guard_dagger_visual`)
- Camera_Follow target dangling 가능성 (memory: `project_camera_follow_dangling`)

→ 이하 §12.1 의 진행 상태 표기는 위 결정 이전 시점 기준이므로, **현 시점의 정설(正說)은 §12.0**.
