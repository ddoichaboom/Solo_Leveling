...
### 7.4 Walk/Run 전이 모델 (2026-04-22 확정)
...
IDLE ──(WASD 누름)──────→ WALK
WALK ──(WASD 뗌)────────→ IDLE
WALK/IDLE ──(SPACE, §4.4.1)─→ DASH / BACK_DASH

DASH/BACK_DASH 종료 시점:
  Run Grace 타이머 시작 (0.3s)

Run Grace 창 (타이머 > 0) 동안:
  WASD 눌림 감지 → RUN 진입 (타이머 초기화 및 소멸)
  WASD 안 눌림  → 타이머 감소, 만료 시 → WALK/IDLE 로 복귀

RUN 상태:
  WASD 유지          → RUN 유지
  WASD 뗌            → IDLE (다시 달리려면 DASH 재입력 필요)
  WASD 방향 급변경    → RUN 유지 (방향만 변경)
  SPACE              → DASH (Priority 상승)
```
...
| 항목 | 결정 |
|---|---|
| **R-1. "특정 시간" 해석** | **Grace window 방식** — DASH 종료 후 0.3s 창 동안 WASD 가 눌리면 RUN. 창 내에 DASH 중 내내 눌려있었던 경우도 포함 |
| **R-2. RUN 중 WASD 뗌** | **IDLE 로 복귀** — 다시 달리려면 DASH 재사용 필요. DASH 의 가치 유지 |
| **R-3. RUN 중 방향 급변경** | **RUN 유지** — 방향만 변경, 상태는 불변 |
| Grace 타이머 초기값 | **0.3s** (실측 후 튜닝) |
| Shift 키 | **QTE 스킬 예약** 유지 (단계 3 비범위). Walk/Run 토글로 사용하지 않음 |

#### 7.4.3 구현 힌트

- `CPlayer_StateMachine` 에 `_float m_fRunGraceTimer`, `_bool m_bInRunGrace` 멤버
- DASH/BACK_DASH 완료 콜백 시점에 `m_fRunGraceTimer = 0.3f`, `m_bInRunGrace = true`
- Update 에서 grace 창 처리: WASD 감지되면 `Try_Transition(RUN)`, 아니면 타이머 감소
- 이동 속도: `_float m_fWalkSpeed`, `_float m_fRunSpeed` 분리. DASH 는 Root Motion 이므로 속도 필드 없음
